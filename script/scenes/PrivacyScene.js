class PrivacyScene extends Phaser.Scene {
	constructor() {
		...
		this.content = [
			...
			['1. INTRODUCTION',
				...
				` WEBSITES PRODUCTS, OFFERS AND SERVICES THAT WE OFFER FROM TIME TO TIME, ALL OF WHICH TOGETHER WE REFER AS ’SERVICES’ OR ’SERVICES’`
			],
			[' '],
			['2. COLLECT OF PERSONAL DATA',
				...
				`PERSONAL DATA MEANS ANY INFORMATION FROM WHICH THE PLAYER CAN BE DIRECTLY	OR INDIRECTLY IDENTIFIED SUCH AS A NAME, LOCATION, EMAIL ADDRESS,	COMMUNICATION LANGUAGE, IP ADDRESS,`,
				`IDFV (ID FOR VENDOR) OR ANDROID ID,	ADVERTISEMENT ID, PUSH NOTIFICATIONS TOKEN, GAME HISTORY, NETWORK LOGS,	COOKIE ONLINE IDENTIFIER, GENDER, FINANCIAL DATA, ETC., (“PERSONAL DATA”).`,
	...
	createTextBlock() {
		...
		let minY = config.scale.height - text.height - 20;

		if (text.height <= config.scale.height - 20) {
